...
void BuddyMemoryAllocator::init_pool() {
  //|1024|512|512|256|256|256|256|...
  size_t pool_blocks = 1 << num_order_;  // pool被划分的块数
  pool_.resize(pool_blocks, 0);
  size_t* pool = pool_.data();           // pool 地址
...
void BuddyMemoryAllocator::update_pool(size_t block_idx) {
  size_t* pool = pool_.data();
...
void* BuddyMemoryAllocator::allocate(size_t size) {
  // 申请的空间大小
  ...
  const auto search_suitable_block = [this, pool](uint32_t target_level,
...
void BuddyMemoryAllocator::deallocate(const void* addr, size_t size) {
  // 计算已申请的内存大小
  const size_t allocate_block_size = rounded_up_to_min_block_size(size, min_block_size_);
  const uint32_t level = compute_level(pool_size_, allocate_block_size);
