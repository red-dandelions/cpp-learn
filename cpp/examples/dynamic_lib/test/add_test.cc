 ...
 * @File:        add_test.cc
 * @Author:      moshouhua
 * @Date:        2022/12/15 23:58:09
 * @Description: 调用动态库的一个例子
 **************************************************************************************************/
...
class Foo {
 public:
  ...
  bool BindFunction(void* dl, const char* name, std::function<T(Params...)>* const func) {
    ...
    // nm add.so | grep add
    // xxxxxxx T __Z7add_ii (这里两道下换线，实际传入dlsym时，要去掉一个下划线) C
    // 语言中 函数名和符号是一样的 void* sym = dlsym(dl, name); // C
    void* sym = dlsym(dl, "_ZN8examples11dynamic_lib3addEii");  // C++
    if (sym == nullptr) {
...
#undef LoadSymbol
  ...
  void Load() {
    void* dl = dlopen(
        "/Users/moshouhua/Desktop/cpp-learn/bazel-bin/cpp/examples/dynamic_lib/add.so", RTLD_LAZY);
    if (dl == nullptr) {
      std::cout << "dl == nullptr\n";
      return;
    }
