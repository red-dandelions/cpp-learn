...
void mem_init() {
  mem_heap = reinterpret_cast<uint8_t*>(malloc(MAX_HEAP)); // 可以用系统调用 sbrk，c 库里的 malloc 也是对 sbrk 进行封装
  mem_brk = mem_heap;
...
// 成功返回 0，否则 -1
int mm_init() {
  if ((heap_listp = reinterpret_cast<uint8_t*>(mem_sbrk(4 * WSIZE))) == reinterpret_cast<uint8_t*>(-1)) {
...
void* mm_malloc(size_t size) {
  ...
  if (size <= DSIZE) {
    ...
    asize = DSIZE * ( (size + (DSIZE) + (DSIZE - 1)) / DSIZE ); // 对齐要求，将申请的内存按两个字对齐，加上头部和脚部占的两个字
  }
